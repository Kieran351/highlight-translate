...
MVP 与可拖动卡片的自动化测试、生产构建检查和 Chrome 界面验收均已通过。2026 年 8 月 15 日还使用实体鼠标/触控板对拖动功能进行了用户复核。

已有记录中的 Chrome 环境仍**没有完成真实 DeepSeek 成功流式请求验证**：设置页连接测试和英文选区都得到扩展归一化后的网络错误。流式处理、SSE 解析、取消、过期结果隔离、超时、部分结果和重试已由可替换 Provider 测试与合成 SSE 浏览器验收覆盖，但这些结果不能证明每台机器上的真实 DeepSeek 请求都能成功。
